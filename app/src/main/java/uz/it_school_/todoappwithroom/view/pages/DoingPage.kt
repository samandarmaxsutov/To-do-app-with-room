...
class DoingPage:Fragment(R.layout.page_doing ){
    private var _binding: PageDoingBinding? = null
    private var itemEdit:((ToDoEntity)->Unit)?=null
    private val binding get() = _binding!!
    private var itemPressListener:((ToDoEntity)->Unit)?=null
    private val navController: NavController by lazy { findNavController() }
    private val viewModel: DoingViewModel by viewModels<DoingViewModelImpl>()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        adapter.removeToDo {
            viewModel.addDone(it)
            viewModel.delete(it.toDoingEntity())
        }
        adapter.itemDelete {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        _binding = PageDoingBinding.inflate(inflater, container, false)
    ...
    private fun deleteDialog(item:ToDoEntity){
        ...
        binding1.yesBtn.setOnClickListener {
            viewModel.delete(item.toDoingEntity())
